    ...
    ContextMixin
{
    ...
    */
    function mintMetaverse(uint256 id)
        external
        payable
        price(getPrice())//(10000000000000000000)
        returns(uint256 index)
    {
        require(!paused(), "MetaX: token mint while paused");
        require(!_exists(id), "MetaX: metaverse id is already minted");
        require(_metasCount < 1000, "MetaX: contract mint limit reached");
        require(id >= 0 && id < 10000, "MetaX: metaverse id does not exists");
